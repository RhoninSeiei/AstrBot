...
def _runner_status(
    response: LLMResponse | None, aborted: bool, errored: bool = False
) -> str:
    if aborted:
        return "aborted"
    if errored or response is None or response.role == "err":
        return "error"
    return "completed"
    ...
    try:
        ...
        await db.insert_provider_stat(
            ...
            status=_runner_status(
                ...
                getattr(agent_runner, "state", None) == AgentState.ERROR,
            ),
