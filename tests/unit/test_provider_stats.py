...
@pytest.mark.asyncio
async def test_error_state_overrides_stale_assistant_response_in_provider_stats():
    db = SimpleNamespace(insert_provider_stat=AsyncMock())
    provider = SimpleNamespace(
        provider_config={"id": "provider-1"},
        meta=lambda: SimpleNamespace(id="provider-1"),
    ...
    runner = SimpleNamespace(
        provider=provider,
        stats=AgentStats(
            token_usage=TokenUsage(input_other=5, output=2),
            start_time=100.0,
            end_time=101.0,
        ),
        provider_stat_segments=[],
    ...
    await record_agent_runner_stats(
        db,
        umo="test:failed-after-partial-response",
        request=None,
        agent_runner=runner,
        final_response=SimpleNamespace(role="assistant"),
    )

    persisted = db.insert_provider_stat.await_args.kwargs
    assert persisted["status"] == "error"
    assert persisted["stats"]["token_usage"] == {
        "input_other": 5,
        "input_cached": 0,
        "output": 2,
    }
